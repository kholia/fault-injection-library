#!/usr/bin/env python3
"""
PY32F003L16S6TU glitch sweep with GPIO-marker classification.

Tuned for demo_py32f003.c auth-bypass target (fault-injection lab firmware).
...
See https://rfcorner.in/posts/voltage-glitching-for-fun-and-profit/ for more
details.

python3 glitch_sweep_py32_gpio.py --glitch-port /dev/tty.usbmodemXXXX

Default marker wiring:
  - Pico Glitcher trigger input <- PY32 PA0 / package pin 2
  - Pico GPIO14               <- PY32 PA1 / package pin 3 (success)
  - Pico GPIO15               <- PY32 PA2 / package pin 4 (failure)
"""
...
class Glitcher:
    ...
    def __init__(
        ...
        marker_reset_active: str,
        marker_pull: str,
        marker_reset_pull: str,
        glitch_mode: str,
        pio_frequency_hz: int,
        ...
        self.marker_reset_level = 0 if marker_reset_active == "low" else 1
        self.marker_pull = marker_pull.upper()
        self.marker_reset_pull = marker_reset_pull.upper()
        self.glitch_mode = glitch_mode
        self.pio_frequency_hz = pio_frequency_hz
    ...
    def connect(self) -> None:
        ...
        reset_pull_expr = f"machine.Pin.PULL_{self.marker_reset_pull}"
        setup_code = f"""
import machine, time
...
_p_r = machine.Pin({self.marker_reset_pin}, machine.Pin.IN, {reset_pull_expr}) if {self.marker_reset_pin} is not None else None
_reset_level = {self.marker_reset_level}

def _poll(timeout_ms):
    deadline = time.ticks_add(time.ticks_ms(), timeout_ms)
    while time.ticks_diff(deadline, time.ticks_ms()) > 0:
        if _p_r is not None and _p_r.value() == _reset_level:
            return "reset"
        success = _p_s is not None and _p_s.value()
        fail = _p_f is not None and _p_f.value()
        if success or fail:
            # Re-sample after sequential GPIO transitions so the dual-high
            # boot marker cannot be mistaken for ordinary success or failure.
            time.sleep_us(10)
            success = _p_s is not None and _p_s.value()
            fail = _p_f is not None and _p_f.value()
        if success and fail:
...
def run_sweep(args: argparse.Namespace) -> int:
    ...
    print(
        f"[i] demo_py32f003.c branch window @ 24 MHz / ~41.7 ns per cycle:\n"
    ...
    g = Glitcher(
        ...
        marker_reset_active=args.reset_active,
        marker_pull=args.marker_pull,
        marker_reset_pull=args.reset_pull,
        glitch_mode=args.glitch_mode,
        pio_frequency_hz=pio_frequency_mhz * 1_000_000,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "PY32F003L16S6TU demo_py32f003.c auth-bypass glitch sweep "
            "(two-phase, GPIO markers)."
        ),
    ...
    hw.add_argument("--success-pin", type=int, default=14,
                     help="Pico GPIO connected to PY32 success marker PA1 (package pin 3).")
    hw.add_argument("--fail-pin", type=int, default=15,
                     help="Pico GPIO connected to PY32 failure marker PA2 (package pin 4).")
    hw.add_argument("--reset-pin", type=int, default=None,
                     help="Optional Pico GPIO monitoring PY32 NRST (package pin 5).")
    hw.add_argument("--reset-active", choices=("low", "high"), default="low",
                     help="Active level of the optional reset marker; PY32 NRST is active-low.")
    hw.add_argument("--marker-pull", choices=("down", "up"), default="down")
    hw.add_argument("--reset-pull", choices=("down", "up"), default="up",
                     help="Pull for optional NRST monitor; pull-up avoids asserting active-low reset.")
    ...
    timing.add_argument("--cooldown-s", type=float, default=0.001,
                        help="Extra host delay; firmware repeats about every 40 ms on failure.")

    storage = p.add_argument_group("storage")
    storage.add_argument("--csv", default="py32f003_glitch_sweep.csv")

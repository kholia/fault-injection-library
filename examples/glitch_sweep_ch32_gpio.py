...
Tuned for the deliberately vulnerable examples/demo.c branch-skip target.

The matching examples/demo.c runs the CH32V003 at 48 MHz (~20.8 ns/cycle)
and places 256 explicit NOPs between the PC1 rising edge and the vulnerable
branch. The NOP sled takes about 5.33 us; argument loads, the function call,
pipeline, GPIO, and trigger-path delays move the practical branch window
slightly later. The default 4.5..6.5 us sweep brackets that window.

Marker protocol:
  - success high, fail low: vulnerable check returned success
  - success low, fail high: vulnerable check returned failure
  - success high, fail high: target booted/rebooted after a brown-out

The sweep uses a two-phase strategy:
...
Efficiency features:
  ...
  - Trigger wait and marker polling execute in one Pico command
  - A dual-high marker distinguishes a reboot from an ordinary failure
...
From the repository root:
  python3 examples/glitch_sweep_ch32_gpio.py --glitch-port /dev/ttyACM0
"""
...
try:
    ...
    if exc.name != "findus":
        raise
    # Permit the documented ``python3 examples/...py`` invocation from a
    # source checkout without requiring an editable installation first.
...
def _poll(timeout_ms):
    ...
    while time.ticks_diff(deadline, time.ticks_ms()) > 0:
        if _p_r and _p_r.value():
            return "reset"
        success = _p_s and _p_s.value()
        fail = _p_f and _p_f.value()
        if success or fail:
            # GPIO writes are sequential. Re-sample after their sub-microsecond
            # transition so the dual-high boot marker cannot be mistaken for a
...
def _wait_and_poll(trigger_timeout_ms, classify_timeout_ms):
    try:
        mp.block(trigger_timeout_ms / 1000)
    except Exception:
        return "no_trigger"
    if not mp.check_glitch():
        return "no_trigger"
    return _poll(classify_timeout_ms)
"""
    ...
    def glitch_and_read_marker(
        self,
        trigger_timeout_ms: int,
        classify_timeout_ms: int,
    ) -> str:
        """Wait, glitch, and classify entirely on the Pico.

        Keeping these operations in one raw-REPL command avoids USB latency
        between the glitch and marker observation.
        """
        try:
            out = self._with_retry(
                self._exec,
                f"print(_wait_and_poll({trigger_timeout_ms}, {classify_timeout_ms}))",
            )
            # PicoGlitcher.check_glitch() also prints its Boolean result. Select
            # the last recognized state rather than depending on exact output.
            valid_states = {
                "success", "fail", "reset", "timeout", "no_trigger",
            }
            for line in reversed(out.decode("utf-8", errors="ignore").splitlines()):
                state = line.strip()
...
def run_coordinate(
    ...
    for _ in range(trials):
        try:
            ...
            state = g.glitch_and_read_marker(
                trigger_timeout_ms=args.trigger_timeout_ms,
                classify_timeout_ms=args.classify_timeout_ms,
            )
            ...
            if early_stop_n > 0 and counts["success"] >= early_stop_n:
                break
...
def run_sweep(args: argparse.Namespace) -> int:
    ...
    print(
        f"[i] demo.c branch windows @ 48 MHz / ~20.8 ns per cycle:\n"
        f"    Explicit pre-branch delay: 256 NOPs = about 5333 ns\n"
        f"    Expected bne window:        roughly 5400-5800 ns\n"
        f"    Default sweep:        {args.offset_start}-{args.offset_stop} ns",
        flush=True,
    )
    if args.trigger_edge != "rising":
        print(
            "[!] The falling edge occurs after vulnerable_check(). "
            "Use --trigger-edge rising to target the intended bne.",
            flush=True,
        )
    if args.offset_step < 4 or args.width_step < 4:
        print(
            "[!] A step below 4 ns oversamples the Pico's 4-5 ns timing "
            "resolution and repeats equivalent hardware coordinates.",
            flush=True,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    hw.add_argument("--glitch-mode", choices=("lp", "hp"), default="lp",
                     help="Start with lp; use hp only if LP cannot produce faults.")
    ...
    hw.add_argument("--success-pin", type=int, default=14,
                     help="Pico GPIO reading CH32 PIN_SUCCESS (PC2).")
    hw.add_argument("--fail-pin", type=int, default=15,
    ...
    # demo.c places 256 explicit NOPs (~5333 ns at 48 MHz) before the argument
    # loads and vulnerable_check call. Bracket the predicted branch window to
    # absorb trigger, GPIO, flash, and pipeline latency.
    coarse.add_argument("--offset-start", type=int, default=4500,
                        help="Start offset in ns from the selected trigger edge.")
    coarse.add_argument("--offset-stop", type=int, default=6500,
                        help="Stop offset in ns (past the predicted bne window).")
    coarse.add_argument("--offset-step", type=int, default=20,
                        help="Coarse offset step in ns (~one 48 MHz cycle).")
    coarse.add_argument("--width-start", type=int, default=5,
                        help="Min glitch width in ns.")
    coarse.add_argument("--width-stop", type=int, default=120,
    ...
    fine.add_argument("--fine-trials", type=int, default=100,
                      help="Trials per coordinate in fine pass (0 to skip phase 2).")
    fine.add_argument("--fine-offset-radius", type=int, default=40,
    ...
    eff.add_argument("--early-stop-n", type=int, default=2,
                     help="Stop a coarse coordinate after N successes; fine coordinates run all trials (0 = disabled).")
    ...
    timing.add_argument("--classify-timeout-ms", type=int, default=50)
    timing.add_argument("--cooldown-s", type=float, default=0.001,
                        help="Host-side inter-trial pause; target attempts repeat after about 40 ms.")

    storage = p.add_argument_group("storage")
    storage.add_argument("--csv", default="ch32v003_demo_nop256_sweep.csv")
...
def _validate_args(
    ...
    if args.offset_stop < args.offset_start:
        parser.error("--offset-stop must be greater than or equal to --offset-start")
    if args.width_stop < args.width_start:
        parser.error("--width-stop must be greater than or equal to --width-start")
    if args.width_start < 5:
        parser.error("--width-start must be at least 5 ns")
